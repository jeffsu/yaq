...
(function() {
  var EventEmitter, Job, LONGTIME, NOOP, PRIORITIES, code, uuid,
  ...
  Job = (function(_super) {
    ...
    Job.prototype.finish = function(error, cb) {
      if (typeof err === 'function') {
        cb = err;
        error = null;
      }
      error || (error = "");
      return this.q.redis["eval"](code.finish, 4, this.q.name, this.id, Date.now(), error, function(err, passed) {
        if (cb) {
          return cb(err || passed !== 1);
        }
      });
    };
